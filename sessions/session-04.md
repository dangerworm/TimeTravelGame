...
- **Stage 0 drone:** Portal is too small/unstable for personal travel. Stage 0 machines send a
  drone: retrieves small quantities of base materials, can write observation papers (Tier 3), no
  artefacts. Stage 1+ = personal travel.
- **Setup variants:** Standard play seeds Stage 0 destination cards to the top of the Modern deck.
  Experienced play removes them; all players start with Stage 1 machines.
- **Researcher shop:** Power Grid-style escalating deck. Each card has a currency cost and a
  reputation threshold. Players without published papers cannot attract late specialists.
...
- **Respect eliminated:** Positive reputation (lending, rescuing, papers) now grants reputation
  points directly. Only disrespect tokens remain.
- **Disrespect tokens:** Each raises the reputation hiring threshold by 1. Decay one per turn
  without ethical violation. Reduce final score by 10 each if held at game end.
- **Deliberate interference:** A special jump variant. Requires a postdoc at base. Exceptional
  material + elevated reputation points. Mandatory Die 1 + Die 2; 2–3 consequence draws, QP cannot
...
5. **Academic scoring metric name** - "reputation points" is the working term; Drew wants something
   h-index-like
...
- **Merit = reputation points** - threshold is a floor not a cost; points never spent. Confirm this
  feels right in play.
