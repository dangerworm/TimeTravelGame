...
1. **Greedy** (P0): max roster, overclock unless next OC = shutdown, deepest era, hire to fill profession gaps.
2. **Cautious** (P1): 1-person roster, never OC, always record (no artefact debt), always Recent era.
3. **Balanced** (P2+): ~60% roster, OC only for 1-shortfall objectives, one era back from max.
...
- Failed MW: −2 integrity. Spec says "4–5" — used 4.
